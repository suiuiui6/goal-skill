...
def main() -> int:
    ...
    errors = []
    if not (root / manifest.get("package_path", "") / "SKILL.md").is_file():
        errors.append("installable Goal package is missing")
    ...
    if errors:
        print("\n".join(errors))
        return 1
    print(f"PASS: {len(entries)} Goal integrations reachable")
    return 0
